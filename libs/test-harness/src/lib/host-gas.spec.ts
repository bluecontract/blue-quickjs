...
describe('host gas', () => {
  ...
  test.each(cases)(
    '$name charges the documented host gas formula',
    (testCase) => {
      ...
      if (!usedMatch || !traceMatch) {
        throw new Error(`Missing gas or trace in output: ${result.stdout}`);
      }
...
function dvLength(expr: string): number {
  ...
  if (!match) {
    throw new Error(`Unexpected dv encode output: ${result.stdout}`);
  }
