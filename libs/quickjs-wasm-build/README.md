...
- Ensure the pinned Emscripten toolchain is installed (`tools/scripts/setup-emsdk.sh`) and the generated QuickJS source tree exists (`bash tools/scripts/prepare-quickjs-source.sh` or `pnpm setup`).
...
- Wasm memory is fixed at 32 MiB (1 MiB stack) with growth disabled; the Emscripten filesystem is stripped (`-sFILESYSTEM=0`), and we pin `SOURCE_DATE_EPOCH=1704067200` to avoid timestamp/env noise in the wasm/loader. We intentionally avoid `-sDETERMINISTIC` because it patches host `Date.now`/`Math.random`.
