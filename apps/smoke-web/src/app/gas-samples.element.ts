...
interface BoundaryFixtureResult {
  name: string;
...
declare global {
  interface Window {
    ...
    __GAS_BOUNDARY_RESULTS__?: BoundaryFixtureResult[];
...
export class GasSamplesElement extends HTMLElement {
  ...
  private async runHarness() {
    ...
    try {
      ...
      const boundaryResults = await runBoundaryFixtureSearches(
        metadata,
        wasmBinary,
      );

      window.__GAS_SAMPLE_RESULTS__ = results;
      window.__GAS_BOUNDARY_RESULTS__ = boundaryResults;
...
async function runBoundaryFixtureSearches(
